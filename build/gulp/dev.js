...
for (var i = 0; i < STYLUS_TASKS.length; i++) {
  (function (i) {
    gulp.task('build-stylus-' + STYLUS_TASKS[i], function () {
      return gulp.src(['src/' + STYLUS_TASKS[i] + '/**/*.css', '!src/' + STYLUS_TASKS[i] + '/**/_*.css'])
        .pipe(gulp.dest('dist/' + STYLUS_TASKS[i]))
        .pipe(livereload())
    })
  })(i)
...
gulp.task('watch', function (event) {
  ...
  for (var i = 0; i < STYLUS_TASKS.length; i++) {
    (function (i) {
      watch('src/' + STYLUS_TASKS[i] + '/**/*.css', function () {
...
gulp.task('dev', ['connect', 'webpack-js', 'move-css', 'move-php', 'build-html', 'build-img', 'watch'], function () {
